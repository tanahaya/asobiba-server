...
cur.execute("""SELECT name, price, category, Evaluation,shopid FROM shop WHERE location = '東京' order by Evaluation desc limit 10 ;""")
...
class MainHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        print self
        content = self.get_argument('content','cont')
        print u"%s が内容です" % (content)
